...
class SubspaceConfig:
    ...
    # --- Expert mode (DMC3 Subspace Trial Parameters Expert) ---
    differencing: bool = False       # first-difference all signals
    double_diff: bool = False        # double-difference (d²/dt²)
    zero_meaning: bool = False       # zero-mean each signal segment
    oversampling_ratio: int = 1      # use every Nth sample
    regularization: float = 0.0      # Tikhonov regularization on Hankel
    cv_group_mode: str = "auto"      # "auto", "one_per_group", "all_in_one"
    max_cv_group_size: int = 10      # max CVs per group
    max_states_per_cv_group: int = 10  # cap model order per group
...
class SubspaceIdentifier:
    ...
    def identify(self, u: Mat, y: Mat) -> SubspaceResult:
        ...
        if cfg.remove_mean or cfg.zero_meaning:
        ...
        # ── Expert mode preprocessing ───────────────────────────────
        if cfg.oversampling_ratio > 1:
            u_proc = u_proc[:, ::cfg.oversampling_ratio]
            y_proc = y_proc[:, ::cfg.oversampling_ratio]
            N = u_proc.shape[1]
            logger.debug("Oversampling: using every %dth sample, N=%d",
                         cfg.oversampling_ratio, N)

        if cfg.differencing:
            u_proc = np.diff(u_proc, axis=1)
            y_proc = np.diff(y_proc, axis=1)
            N = u_proc.shape[1]
            logger.debug("Applied first differencing, N=%d", N)

        if cfg.double_diff:
            u_proc = np.diff(u_proc, n=2, axis=1)
            y_proc = np.diff(y_proc, n=2, axis=1)
            N = u_proc.shape[1]
            logger.debug("Applied double differencing, N=%d", N)
