...
class Layer1TuningPage(QWidget):
    ...
    def _build_ui(self):
        ...
        cv_hint.setStyleSheet("color: #707070; font-size: 8pt;")
        ...
        mv_hint.setStyleSheet("color: #707070; font-size: 8pt;")
        ...
        auto_btn.setStyleSheet(self._button_style("#0099B0"))
        auto_btn.clicked.connect(self._auto_tune)
        btn_row.addWidget(auto_btn)

        reset_btn = QPushButton("Reset to Loaded Values")
        reset_btn.setStyleSheet(self._button_style("#707070"))
    ...
    def _table_style(self):
        return """
        QTableWidget { background: white; alternate-background-color: #F5F5F5;
            border: 1px solid #B0B0B0; gridline-color: #B0B0B0;
            selection-background-color: #0066CC; selection-color: white;
            font-size: 9pt; }
        QHeaderView::section { background: #E4E4E4; border: none;
...
class Layer2SmartTunePage(QWidget):
    ...
    def _build_ui(self):
        ...
        sidebar.setStyleSheet(
            "QFrame { background: #D8D8D8; border-right: 1px solid #ECECEC; }"
        )
        ...
        title.setStyleSheet(
            "color: #404040; font-size: 8pt; font-weight: 600; "
        ...
        QListWidget {
            background: #D8D8D8; border: none; outline: none;
            color: #1A1A1A; font-size: 9pt;
        }
        QListWidget::item {
            padding: 10px 14px; border-bottom: 1px solid #E4E4E4;
        }
        QListWidget::item:selected {
            background: #0066CC; color: white; font-weight: 600;
        ...
        self.help_lbl.setStyleSheet(
            "color: #404040; font-size: 8pt; padding: 8px 12px; "
            "background: #F5F5F5; border-top: 1px solid #ECECEC;")
        ...
        self.stack.setStyleSheet("QStackedWidget { background: #ECECEC; }")
        ...
        self.step6 = Step6_SSCalculator(self.cfg)
...
class Step1_CVRanks(QWidget):
    ...
    def _build(self):
        ...
        QTableWidget { background: white; alternate-background-color: #F5F5F5;
            border: 1px solid #B0B0B0; gridline-color: #B0B0B0;
            selection-background-color: #0066CC; selection-color: white; font-size: 9pt; }
        QHeaderView::section { background: #E4E4E4; border: none;
            border-right: 1px solid #B0B0B0; border-bottom: 1px solid #B0B0B0;
            padding: 5px 8px; font-weight: 600; font-size: 8pt; color: #404040; }
        """)
...
class Step2_Preferences(QWidget):
    ...
    def _build(self):
        ...
        header.setStyleSheet("font-size: 13pt; font-weight: 600; color: #1A1A1A;")
        lay.addWidget(header)
        ...
        QGroupBox { font-weight: bold; font-size: 9pt; color: #1A1A1A;
            border: 1px solid #B0B0B0; border-radius: 4px;
...
class Step4_Evaluate(QWidget):
    ...
    def _build(self):
        ...
        intro = QLabel(
            ...
            "Cells colored <span style='color:#2E8B57'>green</span> show positive gain, "
            "<span style='color:#C0392B'>red</span> show negative gain. "
        ...
        self.table = QTableWidget()
        self.table.setStyleSheet("""
        QTableWidget { background: white; border: 1px solid #B0B0B0;
            gridline-color: #B0B0B0; font-size: 9pt; font-family: Consolas, monospace; }
        QHeaderView::section { background: #E4E4E4; border: none;
            border-right: 1px solid #B0B0B0; border-bottom: 1px solid #B0B0B0;
            padding: 5px 8px; font-weight: 600; font-size: 8pt; color: #404040; }
        """)
        self.table.verticalHeader().setStyleSheet(
            "QHeaderView::section { background: #E4E4E4; padding: 4px 8px; "
            "font-weight: 600; }")
        lay.addWidget(self.table, 1)
...
class Step5_InitTuning(QWidget):
    ...
    def _build(self):
        ...
        QPushButton { background: #0099B0; color: white; border: none;
            border-radius: 4px; padding: 10px 20px; font-size: 10pt; font-weight: 600; }
        QPushButton:hover { background: #0099B0; }
        """)
...
# ============================================================================
# Step 6: Steady-State Calculator
# ============================================================================
class Step6_SSCalculator(QWidget):
    """Offline Layer 2 LP solver: 'what would the SS be right now?'"""
    ...
    def _build(self):
        ...
        header = QLabel("Step 6 — \u26A1 Steady-State Calculator")
        header.setStyleSheet("font-size: 13pt; font-weight: 600; color: #1A1A1A;")
        lay.addWidget(header)

        intro = QLabel(
            "Solve the Layer 2 LP at the current operating point and inspect "
            "where each MV and CV would land. Use this to verify your "
            "preferences and ranks before running the dynamic simulation."
        )
        intro.setWordWrap(True)
        intro.setStyleSheet("color: #707070; font-size: 9pt; padding: 4px 0;")
        lay.addWidget(intro)
        ...
        calc_btn = QPushButton("\u26A1 Calculate Steady State")
        calc_btn.setStyleSheet(self._btn_style("#2E8B57"))
        calc_btn.clicked.connect(self._on_calculate)
        btn_row.addWidget(calc_btn)

        ideal_btn = QPushButton("Calculate Ideal SS (eng limits)")
        ideal_btn.setStyleSheet(self._btn_style("#0066CC"))
        ideal_btn.clicked.connect(self._on_calculate_ideal)
        btn_row.addWidget(ideal_btn)

        clear_btn = QPushButton("Clear")
        clear_btn.setStyleSheet(self._btn_style("#707070"))
        ...
        self.status_chip.setStyleSheet(
            "QLabel { background: #B0B0B0; color: #707070; "
        ...
        # Solver output line
        self.solver_lbl = QLabel("Click 'Calculate Steady State' to solve Layer 2 LP.")
        self.solver_lbl.setStyleSheet(
            "QLabel { background: #D8D8D8; border: 1px solid #B0B0B0; "
            "border-radius: 3px; padding: 6px 10px; "
            "font-family: Consolas, monospace; font-size: 8pt; color: #404040; }")
        lay.addWidget(self.solver_lbl)

        # Splitter with MV (top) and CV (bottom) result tables
        ...
        self.mv_table = QTableWidget()
        self.mv_table.setStyleSheet(self._table_style())
        self.mv_table.setColumnCount(8)
        self.mv_table.setHorizontalHeaderLabels([
            "Tag", "Description", "Current", "SS Value",
            "Op Lo", "Op Hi", "Active", "\u0394 (SS−Cur)"
        ])
        self.mv_table.verticalHeader().setVisible(False)
        self.mv_table.setAlternatingRowColors(True)
        self.mv_table.setEditTriggers(QTableWidget.NoEditTriggers)
        mv_lay.addWidget(self.mv_table)
        split.addWidget(mv_grp)

        # CV results
        cv_grp = QGroupBox("CV Steady-State Results")
        cv_grp.setStyleSheet(self._group_style())
        cv_lay = QVBoxLayout(cv_grp)
        cv_lay.setContentsMargins(8, 16, 8, 8)
        self.cv_table = QTableWidget()
        self.cv_table.setStyleSheet(self._table_style())
        self.cv_table.setColumnCount(8)
        self.cv_table.setHorizontalHeaderLabels([
            "Tag", "Description", "Current", "SS Value",
            "Op Lo", "Op Hi", "Status", "Violation"
        ])
        self.cv_table.verticalHeader().setVisible(False)
        self.cv_table.setAlternatingRowColors(True)
        self.cv_table.setEditTriggers(QTableWidget.NoEditTriggers)
        cv_lay.addWidget(self.cv_table)
        split.addWidget(cv_grp)

        split.setSizes([200, 350])
    ...
    def _populate_initial(self):
        """Show current values with empty SS columns."""
        self.mv_table.setRowCount(len(self.cfg.mvs))
        for r, mv in enumerate(self.cfg.mvs):
            self._set_cell(self.mv_table, r, 0, mv.tag, align=Qt.AlignLeft)
            self._set_cell(self.mv_table, r, 1, mv.name, align=Qt.AlignLeft)
            self._set_cell(self.mv_table, r, 2, f"{mv.value:.3f}")
            self._set_cell(self.mv_table, r, 3, "—")
            self._set_cell(self.mv_table, r, 4, f"{mv.limits.operating_lo:.3f}")
            self._set_cell(self.mv_table, r, 5, f"{mv.limits.operating_hi:.3f}")
            self._set_cell(self.mv_table, r, 6, "—", align=Qt.AlignCenter)
            self._set_cell(self.mv_table, r, 7, "—")
        self.mv_table.resizeColumnsToContents()
        self.mv_table.horizontalHeader().setStretchLastSection(True)

        self.cv_table.setRowCount(len(self.cfg.cvs))
        for r, cv in enumerate(self.cfg.cvs):
            self._set_cell(self.cv_table, r, 0, cv.tag, align=Qt.AlignLeft)
            self._set_cell(self.cv_table, r, 1, cv.name, align=Qt.AlignLeft)
            self._set_cell(self.cv_table, r, 2, f"{cv.value:.3f}")
            self._set_cell(self.cv_table, r, 3, "—")
            self._set_cell(self.cv_table, r, 4, f"{cv.limits.operating_lo:.3f}")
            self._set_cell(self.cv_table, r, 5, f"{cv.limits.operating_hi:.3f}")
            self._set_cell(self.cv_table, r, 6, "—", align=Qt.AlignCenter)
            self._set_cell(self.cv_table, r, 7, "—")
        self.cv_table.resizeColumnsToContents()
        self.cv_table.horizontalHeader().setStretchLastSection(True)

    def _set_cell(self, table, row, col, text, align=None, color=None, bold=False):
    ...
    def _on_calculate(self):
        self._do_calculate(use_engineering=False)

    def _on_calculate_ideal(self):
        self._do_calculate(use_engineering=True)

    def _do_calculate(self, use_engineering: bool):
        # Find the running SimEngine via parent walk
        engine = self._find_engine()
        if engine is None:
            self.solver_lbl.setText(
                "ERROR: No simulation engine available. Apply config first.")
            self._set_status("ERROR", "#C0392B")
            return

        result = engine.compute_ss_target(use_engineering_limits=use_engineering)
        if result is None:
            self.solver_lbl.setText("ERROR: Layer 2 LP not available.")
            self._set_status("ERROR", "#C0392B")
            return

        if use_engineering:
            self._last_ideal = result
        else:
            self._last_result = result

        self._show_result(result, ideal=use_engineering)

    def _show_result(self, result, ideal=False):
        # Status chip
        if result["feasible"]:
            self._set_status("FEASIBLE", "#2E8B57")
        elif result["status"] == "OPTIMAL":
            self._set_status("VIOLATIONS", "#FF8C00")
        else:
            self._set_status(result["status"], "#C0392B")

        # Solver line
        prefix = "IDEAL " if ideal else ""
        self.solver_lbl.setText(
            f"{prefix}SS solve   |   status: {result['status']}   |   "
        ...
        # Update MV table
        u_ss = result["u_ss"]
        u_active = result["u_active"]
        for r, mv in enumerate(self.cfg.mvs):
            ss_val = float(u_ss[r])
            delta = ss_val - mv.value
            active = u_active[r]
            self._set_cell(self.mv_table, r, 2, f"{mv.value:.3f}")
            self._set_cell(self.mv_table, r, 3, f"{ss_val:.3f}", bold=True)
            color = None
            if active == "HI LIM":
                color = QColor("#FFEDD5")
            elif active == "LO LIM":
                color = QColor("#DBEAFE")
            self._set_cell(self.mv_table, r, 6, active,
                           align=Qt.AlignCenter, color=color, bold=True)
            sign = "+" if delta >= 0 else ""
            delta_color = None
            if abs(delta) > 1e-3:
                delta_color = QColor("#E8F5E9") if delta > 0 else QColor("#FFEBEE")
            self._set_cell(self.mv_table, r, 7, f"{sign}{delta:.3f}",
                           color=delta_color)

        # Update CV table
        y_ss = result["y_ss"]
        y_active = result["y_active"]
        for r, cv in enumerate(self.cfg.cvs):
            ss_val = float(y_ss[r])
            active = y_active[r]
            self._set_cell(self.cv_table, r, 2, f"{cv.value:.3f}")
            self._set_cell(self.cv_table, r, 3, f"{ss_val:.3f}", bold=True)
            color = None
            if active.startswith("AT"):
                color = QColor("#FFEDD5")
            elif active.startswith("VIOL"):
                color = QColor("#FFCDD2")
            self._set_cell(self.cv_table, r, 6, active,
                           align=Qt.AlignCenter, color=color, bold=True)
            # Violation amount
            viol = ""
            for vi, va in result["violations"]:
                if vi == r:
                    viol = f"\u00B1{va:.3f}"
                    break
            self._set_cell(self.cv_table, r, 7, viol or "—",
                           color=QColor("#FFEBEE") if viol else None)
    ...
    def _on_clear(self):
        self._populate_initial()
        self.solver_lbl.setText("Click 'Calculate Steady State' to solve Layer 2 LP.")
        self._set_status("READY", "#707070")

    def _find_engine(self):
        """Walk up the parent chain to find the SimEngine via the MainWindow."""
        w = self
        while w is not None:
            if hasattr(w, "simulator") and hasattr(w.simulator, "engine"):
                return w.simulator.engine
            w = w.parent() if hasattr(w, "parent") else None
        return None

    def refresh(self):
        # Refresh current-value columns
        for r, mv in enumerate(self.cfg.mvs):
            if r < self.mv_table.rowCount():
                self._set_cell(self.mv_table, r, 2, f"{mv.value:.3f}")
                self._set_cell(self.mv_table, r, 4, f"{mv.limits.operating_lo:.3f}")
                self._set_cell(self.mv_table, r, 5, f"{mv.limits.operating_hi:.3f}")
        for r, cv in enumerate(self.cfg.cvs):
            if r < self.cv_table.rowCount():
                self._set_cell(self.cv_table, r, 2, f"{cv.value:.3f}")
                self._set_cell(self.cv_table, r, 4, f"{cv.limits.operating_lo:.3f}")
                self._set_cell(self.cv_table, r, 5, f"{cv.limits.operating_hi:.3f}")
...
class Layer3ConfigPage(QWidget):
    ...
    def _build(self):
        ...
        self.enable_chk.setStyleSheet(
            "QCheckBox { font-weight: 600; font-size: 10pt; color: #1A1A1A; }"
    ...
    def show_rto_result(self, result):
        ...
        if not result.success:
            self.result_lbl.setText(
                f"<b style='color:#C0392B'>RTO FAILED</b>: {result.message}\n"
        ...
        self.result_lbl.setText(
            f"<b style='color:#2E8B57'>RTO SUCCESS</b>  ({result.message})\n"
...
class OptimizerWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        action_bar.setStyleSheet(
            "QFrame { background: #D8D8D8; border-top: 1px solid #B0B0B0; }")
        ...
        self.dirty_lbl.setStyleSheet(
            "color: #C0392B; font-size: 8pt; font-weight: 600;")
